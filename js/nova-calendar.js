...
(function (window, $, lv, nova) {
    ...
    Calendar.prototype = {
        ...
        destroyHandler: function (e) {
            ...
            var $calendar = $target.parents(".nova-calendar");
        ...
        triggerEventHandler: function (e) {
            ...
            $('.nova-calendar[data-float=true]').remove();
        ...
        bindBirthdayEvent: function () {
            ...
            this.wrap.off("click", ".nova-calendar-year-select", this.yearDropdownHandler);
            this.wrap.on("click", ".nova-calendar-year-select", {self: this}, this.yearDropdownHandler);

            this.wrap.off("click", ".nova-calendar-month-select", this.monthDropdownHandler);
            this.wrap.on("click", ".nova-calendar-month-select", {self: this}, this.monthDropdownHandler);

            this.wrap.off("click", ".nova-calendar-year-dropdown li", this.yearSelectHandler);
            this.wrap.on("click", ".nova-calendar-year-dropdown li", {self: this}, this.yearSelectHandler);

            this.wrap.off("click", ".nova-calendar-month-dropdown li", this.monthSelectHandler);
            this.wrap.on("click", ".nova-calendar-month-dropdown li", {self: this}, this.monthSelectHandler);

            this.wrap.off("click", ".nova-calendar-birthday-clear", this.birthdayClearHandler);
            this.wrap.on("click", ".nova-calendar-birthday-clear", {self: this}, this.birthdayClearHandler);

            this.wrap.off("click", ".nova-calendar-birthday-ok", this.birthdayOkHandler);
            this.wrap.on("click", ".nova-calendar-birthday-ok", {self: this}, this.birthdayOkHandler);

            this.wrap.off("click", ".nova-calendar-hour", this.birthdayHourHandler);
            this.wrap.on("click", ".nova-calendar-hour", {self: this}, this.birthdayHourHandler);

            this.wrap.off("click", ".nova-calendar-minute", this.birthdayMinuteHandler);
            this.wrap.on("click", ".nova-calendar-minute", {self: this}, this.birthdayMinuteHandler);

            this.wrap.off("click", ".nova-calendar-hour-dropdown li", this.hourSelectHandler);
            this.wrap.on("click", ".nova-calendar-hour-dropdown li", {self: this}, this.hourSelectHandler);

            this.wrap.off("click", ".nova-calendar-minute-dropdown li", this.minuteSelectHandler);
            this.wrap.on("click", ".nova-calendar-minute-dropdown li", {self: this}, this.minuteSelectHandler);

            this.wrap.off("click", ".nova-calendar-minute-dropdown .close", this.minuteCloseHandler);
            this.wrap.on("click", ".nova-calendar-minute-dropdown .close", {self: this}, this.minuteCloseHandler);

            this.wrap.off("click", ".nova-calendar-hour-dropdown .close", this.hourCloseHandler);
            this.wrap.on("click", ".nova-calendar-hour-dropdown .close", {self: this}, this.hourCloseHandler);
        },

        hourCloseHandler: function (e) {
            e.stopPropagation();
            var self = e.data.self;
            var $hourDropdown = self.wrap.find(".nova-calendar-hour-dropdown");
            $hourDropdown.remove();
            self.wrap.find(".nova-calendar-hour").removeClass("active");
        },

        minuteCloseHandler: function (e) {
            e.stopPropagation();
            var self = e.data.self;
            var $minuteDropdown = self.wrap.find(".nova-calendar-minute-dropdown");
            $minuteDropdown.remove();
            self.wrap.find(".nova-calendar-minute").removeClass("active");
        },
        ...
        hourSelectHandler: function (e) {
            ...
            var $calHour = self.wrap.find(".nova-calendar-hour");
        ...
        minuteSelectHandler: function (e) {
            ...
            var $calMinute = self.wrap.find(".nova-calendar-minute");
        ...
        birthdayHourHandler: function (e) {
            ...
            self.wrap.find(".nova-calendar-hour-dropdown").remove();
            self.wrap.find(".nova-calendar-minute-dropdown").remove();

            var $hourDropdown = $('' +
                '<div class="nova-calendar-hour-dropdown">' +
        ...
        birthdayMinuteHandler: function (e) {
            ...
            var $minuteDropdown = $('' +
                '<div class="nova-calendar-minute-dropdown">' +
        ...
        hideDropdownHandler: function (e) {
            ...
            var $yearDropdown = self.wrap.find(".nova-calendar-year-dropdown-box");
            var $monthDropdown = self.wrap.find(".nova-calendar-month-dropdown-box");
            var $hourDropdown = self.wrap.find(".nova-calendar-hour-dropdown");
            var $minuteDropdown = self.wrap.find(".nova-calendar-minute-dropdown");
            var $hour = self.wrap.find(".nova-calendar-hour");
            var $minute = self.wrap.find(".nova-calendar-minute");
            ...
            if ($target.is(".nova-calendar-year-select") || $target.parents(".nova-calendar-year-select").length > 0) {
                $yearDropdown.show();
            }

            if ($target.is(".nova-calendar-month-select") || $target.parents(".nova-calendar-month-select").length > 0) {
                $monthDropdown.show();
            }

            if ($target.is(".nova-calendar-hour") || $target.is(".nova-calendar-hour-dropdown") || $target.parents(".nova-calendar-hour-dropdown").length > 0) {
                $hourDropdown.show();
                $hour.addClass("active");
            }

            if ($target.is(".nova-calendar-minute") || $target.is(".nova-calendar-minute-dropdown") || $target.parents(".nova-calendar-minute-dropdown").length > 0) {
                $minuteDropdown.show();
        ...
        yearDropdownHandler: function (e) {
            ...
            $yearDropdown.find(".nova-calendar-year-dropdown").scrollTop(currentLiTop)
        ...
        bindMonthChangeEvent: function () {
            ...
            var $monthPrev = this.wrap.find(".nova-calendar-month-prev");
            var $monthNext = this.wrap.find(".nova-calendar-month-next");
        ...
        hideMonthNext: function () {
            this.wrap.find(".nova-calendar-month-next").hide();
        },

        hideMonthPrev: function () {
            this.wrap.find(".nova-calendar-month-prev").hide();
        },

        showMonthNext: function () {
            this.wrap.find(".nova-calendar-month-next").show();
        },

        showMonthPrev: function () {
            this.wrap.find(".nova-calendar-month-prev").show();
        },
        ...
        hideNumberOfDays: function () {
            $(".nova-calendar-number-of-days").hide();
        },
        ...
        render: function (disableCallback) {
            ...
            if (this.wrap.is(".nova-calendar-big") && this.options.showPriceTip) {
        ...
        renderTime: function () {
            ...
            if (this.options.hasTime) {
                this.wrap.find(".nova-calendar-pane").children().css({
        ...
        setTimeHtml: function (time) {
            ...
            var $hour = this.wrap.find(".nova-calendar-hour");
            var $minute = this.wrap.find(".nova-calendar-minute");
                ...
                if (this.cascadingIndex === 0) {
                    ...
                    if (start) {
                        ...
                        if (end && $endDate.length > 0) {
                            ...
                            var $numberOfDays = $(".nova-calendar-number-of-days");

                            if ($numberOfDays.length <= 0) {
                                $numberOfDays = $('' +
                                    '<div class="nova-calendar-number-of-days">' +
        ...
        createSingleCalendar: function (date, offset) {
            ...
            if (this.options.weekHeader) {
                weekHeaderTh = '<th class="nova-calendar-week-header"></th>';
            }
            ...
            if (this.options.division) {
                ...
                nextMonthTitle = nextMonthTitle.replace("nova-calendar-title", "nova-calendar-title nova-calendar-title-next");
                html += nextMonthTitle
            }
        ...
        createDate: function (date) {
            ...
            dates.forEach(function (row, index) {
                dateHtml = "";
                if (weekHeader) {
                    dateHtml += "<td class='nova-calendar-week-header'></td>";
                }
                ...
                if (weekInterval) {
                    if (dateSize - 1 === index) {
                        ...
                        html += '<tr><td class="nova-calendar-week-interval" colspan="8"></td></tr>'
                    }
                }
            });
    ...
    //公用模板
    var calControl = '<span class="nova-calendar-month-prev" title="上一月">‹</span><span class="nova-calendar-month-next" title="下一月">›</span>';
    var calWrap = '<div class="calwrap clearfix">{{content}}</div>';
    var calMonth = '<div class="nova-calendar-month" {{offset}}>{{content}}</div>';
    var calTitle = '<div class="nova-calendar-title"><span class="mtitle">{{month}}</span></div>';
    var calTable = '<table class="nova-calendar-table">' +
        '    <thead>' +
        '        <tr>' +
        '            {{weekHeader}}' +
    ...
    //小日历模板
    var smallTemplate = {
        wrap: '<div class="nova-calendar nova-calendar-mini" {{bimonthly}} {{float}}></div>',
    ...
    //大日历模板
    var bigTemplate = {
        wrap: '<div class="nova-calendar nova-calendar-big" {{bimonthly}} {{float}}></div>',
        calControl: calControl,
        calWrap: calWrap,
        calMonth: calMonth,
        calTitle: calTitle,
    ...
    var birthdayTemplate = {
        wrap: '<div class="nova-calendar nova-calendar-mini nova-calendar-birthday" {{bimonthly}} {{float}}></div>',
        calControl: calControl,
        calWrap: calWrap,
        calMonth: calMonth,
        calTitle: calTitle,
        ...
        '<div class="nova-calendar-box">' +
        calTable +
        '<div class="nova-calendar-pane">' +
        '<span class="nova-calendar-time-text">时间</span>' +
        '<span class="nova-calendar-time-input">' +
        '<span class="nova-calendar-hour">00</span><span class="nova-calendar-time-to">:</span><span class="nova-calendar-minute">00</span>' +
        '</span>' +
        '<span class="btn btn-xs nova-calendar-birthday-clear">清空</span>' +
        '<span class="btn btn-xs nova-calendar-birthday-ok">确定</span>' +
        '</div>' +
        '<div class="month-loading"></div>' +
        '</div>',
        weekWrap: weekWrap,
